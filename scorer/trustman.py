    ...
    for node in dataHist:
        ...
        if (dataHist[node]["cpu_score"] != None and dataHist[node]["ram_score"] != None):
            ...
            if (int(node) == indice_best_cpu):
                new_score_cpu = Augmentation_score(
                    float(dataHist[str(indice_best_cpu)]["cpu_score"]), best_resource_cpu)
            else:
                # diminution
                new_score_cpu = Dimunition_score(float(dataHist[node]["cpu_score"]), float(
                    dataHist[node]["cpu"][-1]), best_resource_cpu)

            if (int(node) == indice_best_ram):
                # Mettre à jour le score des nodes
                new_score_ram = Augmentation_score(
                    float(dataHist[str(indice_best_ram)]["ram_score"]), best_resource_ram)
            else:
                new_score_ram = Dimunition_score(float(dataHist[node]["ram_score"]), float(
                    dataHist[node]["ram"][-1]), best_resource_ram)
        else:
            new_score_cpu = 0.5
            new_score_ram = 0.5
